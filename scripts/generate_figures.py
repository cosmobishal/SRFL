...
============================
Standalone script to reproduce all 7 publication figures.
Works for any --steps value >= 8.

Usage
-----
    python scripts/generate_figures.py
    python scripts/generate_figures.py --outdir figures --n 512 --steps 70
    python scripts/generate_figures.py --n 128 --steps 20 --dpi 72   # fast smoke test
"""
...
def parse_args():
    ...
    p.add_argument("--steps",  type=int, default=70,     help="Scale steps S (>= 8)")
...
def _prop_ids(steps, count=3):
    """Return `count` evenly-spaced step indices safe for any steps value."""
    return [int(round(i * (steps - 1) / (count - 1))) for i in range(count)]
...
def main():
    ...
    def step_fn(xv):
        return np.where(xv >= 0, 1.0, 0.0).astype(float)

    def osc_fn(xv):
        out = np.zeros_like(xv, dtype=float)
        nz  = xv != 0
        out[nz] = xv[nz] * np.sin(1.0 / xv[nz])
        return out
    ...
    f_step, e_step = SRFLField(x, t_step, lam_sched).run()

    print("  Running oscillatory field evolution ...")
    f_osc, e_osc = SRFLField(x, t_osc, lam_sched).run()
    ...
    plt.close()
    print("  OK  field_evolution.png")

    # -- Figure 2: swarm_structure ------------------------------------------
    n_plot = min(len(sw_step.history), S)
    fig, ax = plt.subplots(figsize=(12, 7))
    for s in range(0, n_plot, max(1, n_plot // 40)):
        pos = sw_step.history[s]
        c   = s / max(n_plot - 1, 1)
        ax.scatter(pos, np.full(len(pos), s), s=22,
                   color=plt.cm.plasma(1 - c), alpha=0.75, zorder=3)
    for s in range(0, n_plot - 1, max(1, n_plot // 15)):
        pos = sw_step.history[s]
    ...
    for s_ev, etype, ex in sw_step.events:
        if s_ev >= n_plot:
            continue
        m = {"spawn": "^", "merge": "D", "annihilate": "x"}[etype]
        c = {"spawn": "lime", "merge": "gold", "annihilate": "red"}[etype]
        ax.scatter(ex, s_ev, marker=m, s=90, color=c, zorder=5)
    ax.set_xlabel("$x$")
    ax.set_ylabel("Scale step $s$  (coarse to fine)")
    ...
    ax.set_xlabel("$x$")
    ax.set_ylabel("$K$ (normalised)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.25)
    ax = axes[1]
    step_s = max(1, args.n // 120)
    xs = x[::step_s]
    ...
    plt.close()
    print("  OK  nonlocal_kernel.png")

    # -- Figure 4: step_defect ----------------------------------------------
    fig, axes = plt.subplots(2, 3, figsize=(15, 9))
    stage_ids = np.linspace(0, S - 1, 6, dtype=int)
    ...
    plt.close()
    print("  OK  oscillation_defect.png")
    ...
    plt.close()
    print("  OK  swarm_dynamics.png")

    # -- Figure 7: multiscale_flow -----------------------------------------
    # Use proportional indices so this works for any S >= 8
    ids  = _prop_ids(S, count=3)
    fig, axes = plt.subplots(1, 3, figsize=(15, 6))
    ax = axes[0]
    field_mat = np.array(f_step)
    im = ax.imshow(field_mat, extent=[x[0], x[-1], S - 1, 0],
    ...
    for i_id, col, lab in zip(ids, cols, labs):
        ax.plot(x, f_step[i_id], color=col, lw=2.4, label=lab)
    ...
    ax.invert_xaxis()
    ax.set_xlabel("$\\lambda$")
    ax.set_ylabel(r"$\|\Phi-f\|_{L^2}$")
    ax.set_title("Convergence: Error vs $\\lambda$")
    ax.legend(fontsize=9)
    ax.grid(True, which="both", alpha=0.25)
    plt.suptitle(
        r"Multi-scale Flow: $\Pi(\lambda_1\to\lambda_2)$, Scale Consistency",
        fontsize=13)
    plt.tight_layout(rect=[0, 0, 1, 0.92])
    plt.savefig(f"{args.outdir}/multiscale_flow.png", dpi=args.dpi, bbox_inches="tight")
    plt.close()
    print("  OK  multiscale_flow.png")
